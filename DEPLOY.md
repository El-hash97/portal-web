...
- `DEVELOPER_PASSWORD` — **baru**, wajib ditambahkan manual. Password yang dipakai untuk menandai request "Dikerjakan"/"Selesai" di halaman `/open-request` — otomatis terisi dari sesi login admin yang sudah ada, developer tidak perlu mengetik ulang. **Penting:** nilai `DEVELOPER_PASSWORD` harus persis sama dengan `ADMIN_CRED.pass` di `src/lib/constants.ts` — mekanisme "tidak perlu mengetik ulang" bekerja dengan mengirim ulang password yang sama dipakai saat login admin, jadi kalau kedua nilai ini berbeda, tombol "Mulai Kerjakan"/"Tandai Selesai" akan selalu gagal dengan error 401 meskipun sudah login sebagai admin.
