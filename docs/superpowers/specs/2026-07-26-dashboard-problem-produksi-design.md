...
- Stack totals: the `<Bar>` for the **last** line in `lines` carries the label for its stack — `<LabelList dataKey="opTotal" position="top">` on the last `op:` bar, `<LabelList dataKey="finTotal" position="top">` on the last `fin:` bar. Because that bar sits at the cumulative top of its stack, `position="top"` lands the label above the whole stack even when that particular line contributes zero for the month (a zero-height segment is still positioned at the stack's running total). Labels use a formatter returning an empty string when the total is `0`, so empty months stay clean instead of showing a row of zeros.
